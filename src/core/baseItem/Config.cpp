...
long long Config::getLong(const std::string& name, long long defaultValue) {
    // 如果传入的是默认值0，尝试使用setifno设置的默认值
    long long actualDefaultValue = defaultValue;
    if (defaultValue == 0) {
        auto defaultIt = defaultValues.find(name);
        if (defaultIt != defaultValues.end()) {
            try {
                actualDefaultValue = std::stoll(defaultIt->second);
            } catch (const std::exception& e) {
                std::cerr << "Error converting default value to long long: " << e.what() << std::endl;
                actualDefaultValue = defaultValue;
            }
        }
    }
    ...
    try {
        ...
        std::cerr << "Error converting value to long long: " << e.what() << std::endl;
        return actualDefaultValue;
    }
}
...
void Config::setifno(const std::string& name, const Region& region) {
    std::string key=name;
    if(name.find(".") == std::string::npos)key = name;
